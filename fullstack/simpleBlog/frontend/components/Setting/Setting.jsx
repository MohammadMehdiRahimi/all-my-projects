...
export default function Setting() {
  ...
  const { userId } = useSelector((state) => state.profileDetails);
  ...
  const formSubmit = async (e) => {
    ...
    if (passwordVal.current.value !== confirmPasswordVal.current.value) {
      ...
      if (aboutVal.current.value !== "") {
        formData.append("aboutMe", aboutVal.current.value);
      }
      if (userId !== "") formData.append("userId", userId);
      if (!!newImageFile.current.files[0]) {
        formData.append("profileImg", newImageFile.current.files[0]);
      }
      try {
  ...
  useEffect(() => {
    const getDetails = async () => {
      try {
        ...
        if (data.success) {
          userNameVal.current.value = data.body.userName;
          emailVal.current.value = data.body.email;
          aboutVal.current.value = data.body.aboutMe;
          profile.current.src = `${constance.baseUrl}/profileImage/${data.body.profileImg}`;
        }
      } catch (error) {
          ...
          >
            <div className={"mb-5  " + style.inputGroup}>
              ...
              <div className={"  " + style.email}>
                ...
                <input type="email" id="email" ref={emailVal} disabled />
              </div>
