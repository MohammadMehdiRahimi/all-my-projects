...
export default class Controller {
  static async register(req, res) {
    ...
    try {
      ...
      if (addStatus.success) {
        try {
          const token = await Models.getToken(addStatus.body.id);
          return res.json({
            success: true,
            body: { id: addStatus.body.id, token },
  ...
  static async getDetails(req, res) {
    ...
    if (userId) {
      try {
        ...
        // if (user.body[0].profileImg === null) {
        //   user.body[0].profileImg = "global.png";
        // }

        return res.json({ success: true, body: user.body[0] });
      } catch (error) {
        return res.json(error);
      }
    } else {
  ...
  static async updateProfile(req, res) {
    ...
    if (profileImg) {
      myObject.profileImg = profileImg;
    }
